...
module.exports={
    save(req,res){
        ...
        image = data.image
        var sql = 'INSERT INTO artist (name, description,image) VALUES ("'+name+'","'+description+'","'+image+'")';
    ...
    update(req,res){
        ...
        image = data.image
        console.log(data);
        conexion.query('UPDATE artist SET (name,description,image) VALUES ("'+name+'","'+description+'","'+image+'")')
    },
   update(req, res) {
        id = req.params.id;
        data = req.body;
   ...
   uploadimage(req, res) {
    ...
    if (req.files) {
        ...
        if (file_ext == 'jpg' || file_ext == 'gif' || file_ext == 'png' || file_ext == 'jpeg') {
            const sql = 'UPDATE artist SET image=? WHERE id=?';

            conexion.query(sql, [file_name, id], function (err, results, fields) {
                if (!err) {
                    if (results.affectedRows != 0) {
                        res.status(200).send({ message: 'Imagen actualizada correctamente' });
                    } else {
                        res.status(200).send({ message: 'No se actualizó ninguna fila. Puede que el ID no exista.' });
                    }
                } else {
                    console.error(err);
            ...
            res.status(400).send({ message: 'Formato de imagen no válido. Solo se permiten archivos JPG, GIF, PNG, y JPEG.' });
        }
    } else {
